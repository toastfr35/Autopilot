...
package body AFDS.pitch is
   ...
   emergency_pitch_angle : t_pitch := 0.0;  -- for GCAS
   ...
   -------------------------------
   procedure set_target (pitch_angle : t_pitch) is
   begin
   ...
   -------------------------------
   procedure set_emergency_override (enabled : Boolean; pitch_angle : t_pitch) is
   begin
   ...
   -------------------------------
   procedure set_roll_elevator (v  : t_elevator) is
   begin
      roll_elevator := v;
   ...
   begin
      if emergency_enabled then
         AFDS.iface.ACC.data.target_pitch := emergency_pitch_angle;
         return t_pitch_correction (emergency_pitch_angle - AFDS.iface.ACS.data.pitch);
      else
         AFDS.iface.ACC.data.target_pitch := target_pitch_angle;
         return t_pitch_correction (target_pitch_angle - AFDS.iface.ACS.data.pitch);
      end if;
   ...
   begin
      ...
      if fabs (Float (AFDS.iface.ACS.data.roll)) < 90.0 then
         G_roll := 3.0 - 2.0 * fcos (fabs (Float (AFDS.iface.ACS.data.roll) * Pi / 180.0));
      else
         G_roll := 1.0;
      end if;
      ...
      AFDS.elevator.set_target (t_elevator(target_elevator));
   end step;
