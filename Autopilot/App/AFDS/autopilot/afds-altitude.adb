...
package body AFDS.altitude is
   ...
   -------------------------------
   subtype t_vspeed_limit is t_vspeed range -150.0 ..150.0;


   -------------------------------
   -- Lookup Table for altitude correction -> vertical speed
   -------------------------------
   package p_LUT is new lookuptable (t_altitude_correction, t_vspeed);
   ...
   -- for detecting change in altitude target
   ...
   function altitude_correction return t_altitude_correction is
      a : constant t_altitude := AFDS.iface.GPS.data.altitude;
      a_t : constant t_altitude := AFDS.iface.AFDS.data.nav_target.altitude;
   begin
   ...
   begin
      ...
      target_vspeed := t_vspeed'Max (target_vspeed, t_vspeed_limit'First);
      target_vspeed := t_vspeed'Min (target_vspeed, t_vspeed_limit'Last);
