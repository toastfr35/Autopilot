...
package body auto_heading is
   ...
   function heading_correction return t_heading_correction is
      current : constant t_heading := aircraft.status.heading;
      target : constant t_heading := nav_AFDS.get_heading;
      error : t_heading;
   ...
   begin
      ...
      auto_pitch.set_roll_elevator (target_elevator);
      ...
      --pragma Debug (log.log (log.heading, 10, 0, "Heading (+-)" & img.Image(Float(abs_correction)) & "  Roll:" & img.Image(target_roll) & "  Rudder:" & img.Image(Float(target_rudder))));

      ctrl_rudder.set_target(target_rudder);
      auto_roll.set_target (target_roll);
   end step;
